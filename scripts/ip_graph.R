...
ip_plt <- ggplot(plot_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "Immunopositive Cell Densitiy 1B, 7B, 11B", y = "Mean Cell Density") +
  theme(
...
ggsave(filename = "analysis/bax_ip_1B_7B-11B.png", plot = ip_plt, 
       width = 12, height = 10, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/bax_ip_1B_7B-11B.rds")
