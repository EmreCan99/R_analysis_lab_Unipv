...
data_combined_1A <- bind_rows(
  lapply(names(gl_casp3.g[[ab]]), function(name) {
...
ggplot(data_combined_1A, aes(x = bar_name, Mean, color = source)) + 
...
# View result
print(data_combined_1A)

# Separate outliers
outliers <- data_combined_1A %>% filter(outlier)
non_outliers <- data_combined_1A %>% filter(!outlier)
...
# result_list <- split(data_combined_1A, data_combined_1A$source)