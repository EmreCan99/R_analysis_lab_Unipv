...
ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  theme_minimal() +
  labs(title = "One-Way ANOVA Results - casp3 1B, 7B, 11B", y = "op-Mean", x = "Condition")
...
padding <- 26 # Add some padding above the maximum value STANDART 22
...
plt <- ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "casp3 1B, 7B, 11B", y = "Gralunular Cells (OD)", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/g_op_plt/g_casp3_1B_7B_11B.png", plot = plt, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt, file = "analysis/g_casp3_1B_7B_11B.rds")


ggsave(filename = "analysis/g_op_plt/g_casp3_1B_7B_11B.sig.png", plot = plt.sig, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/g_casp3_1B_7B_11B.sig.rds")
