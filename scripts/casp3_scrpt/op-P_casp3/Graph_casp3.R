...
ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  theme_minimal() +
  labs(title = "One-Way ANOVA Results - casp3 1A, 7A, 11A", y = "op-Mean", x = "Condition")
...
padding <- 23 # Add some padding above the maximum value STANDART 10
...
plt <- ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "casp3 1A, 7A, 11A", y = "Purkinje (OD)", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/casp3_1A_7A_11A.png", plot = plt, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt, file = "analysis/casp3_1A_7A_11A.rds")


ggsave(filename = "analysis/op_plt/casp3_1A_7A_11A.sig.png", plot = plt.sig, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/casp3_1A_7A_11A.sig.rds")
