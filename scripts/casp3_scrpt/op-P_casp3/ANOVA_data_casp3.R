...
cnt_combined <- bind_rows(
  lapply(names(gl_casp3.p$l_1A), function(name) {
    if (name != "Normality"){
    gl_casp3.p$l_1A[[name]]}
  }),
...
trt1_combined <- bind_rows(
  lapply(names(gl_casp3.p$l_7A), function(name) {
    if (name != "Normality"){
    gl_casp3.p$l_7A[[name]]}
  }),
  .id = "source"  # Add "source" column to track origin
)
# EXCLUDE THE AREAS
...
trt2_combined <- bind_rows(
  lapply(names(gl_casp3.p$l_11A), function(name) {
    if (name != "Normality"){
    gl_casp3.p$l_11A[[name]]}
  }),
  .id = "source"  # Add "source" column to track origin
)
# EXCLUDE THE AREAS
...
saveRDS(set_combined, file = "analysis/raw_rds/casp3_1A_7A_11A.op.p.rds")
