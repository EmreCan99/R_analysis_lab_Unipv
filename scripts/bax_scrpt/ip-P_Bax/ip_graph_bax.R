...
padding <- 0.015 # Add some padding above the maximum value STANDART 0
pmin <- 3.4
...
plt <- ggplot(plot_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "Bax 1B, 7B, 11B", y = "Purkinje Cell Density", x = "Condition") +
  theme_classic() +
...
hgt2 <- hgt - hgt/22 # to add all make it 7 and for two make 22
dht <- hgt/25
size <- 0.5
...
ggsave(filename = "analysis/ip_plt/bax_ip_1B_7B_11B.png", plot = plt.sig, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt.sigx3, file = "analysis/bax_ip_1B_7B_11B.rds")
