...
padding <- 0.040 # Add some padding above the maximum value STANDART 0
pmin <- 3.4
...
plt <- ggplot(plot_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "Bax 1B, 7B, 11B", y = "Granular Cell (CD)", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/g_ip_plt/bax_ip_1B_7B_11B.png", plot = plt.sig, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/bax_ip_1A_7A_11A.rds")
