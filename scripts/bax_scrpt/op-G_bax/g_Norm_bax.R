...
if (!"Normality" %in% names(df)) {
  gl_bax.g$l_11B$Normality <- data.frame(Shapiro = NA_character_, stringsAsFactors = FALSE)
}
gl_bax.g$l_11B$Normality$Shapiro <- ifelse(result_sh > alpha, "passed", "NOT passed")
...
gl_bax.g$l_11B$Normality <- gl_bax.g$l_11B$Normality %>%
...
gl_bax.g$l_11B$Normality <- rbind(gl_bax.g$l_11B$Normality, tempdf)
