...
ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  theme_minimal() +
  labs(title = "One-Way ANOVA Results - Bax 1B, 7B, 11B", y = "op-Mean", x = "Condition")
...
padding <- 40 # Add some padding above the maximum value
...
plt <- ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "g-bax 1B, 7B, 11B", y = "Mean Gray-Value", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/g_bax_1B_7B_11B.png", plot = plt, 
       width = 12, height = 10, dpi = 300, units = "cm")

saveRDS(plt, file = "analysis/g_bax_1B_7B_11B.rds")


ggsave(filename = "analysis/g_bax_1B_7B_11B.sig.png", plot = plt.sig, 
       width = 12, height = 10, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/g_bax_1B_7B_11B.sig.rds")
