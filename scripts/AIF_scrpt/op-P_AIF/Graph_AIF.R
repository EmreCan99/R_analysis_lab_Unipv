...
padding <- 17 # Add some padding above the maximum value STANDART 10
...
plt <- ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "AIF 1A, 7A, 11A", y = "Purkinje (OD)", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/op_plt/AIF_1A_7A_11A.png", plot = plt, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt, file = "analysis/AIF_1A_7A_11A.rds")


ggsave(filename = "analysis/op_plt/AIF_1A_7A_11A.sig.png", plot = plt.sig, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/AIF_1A_7A_11A.sig.rds")
