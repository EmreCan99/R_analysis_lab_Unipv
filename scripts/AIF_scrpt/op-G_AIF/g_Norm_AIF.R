...
# EXCLUDE THE AREAS
data_combined_11B <- data_combined_11B[data_combined_11B$X != 1 & data_combined_11B$X != 2, ]
...
if (!"Normality" %in% names(df)) {
  gl_AIF.g$l_11B$Normality <- data.frame(Shapiro = NA_character_, stringsAsFactors = FALSE)
}
gl_AIF.g$l_11B$Normality$Shapiro <- ifelse(result_sh > alpha, "passed", "NOT passed")
...
gl_AIF.g$l_11B$Normality <- gl_AIF.g$l_11B$Normality %>%
...
gl_AIF.g$l_11B$Normality <- rbind(gl_AIF.g$l_11B$Normality, tempdf)
