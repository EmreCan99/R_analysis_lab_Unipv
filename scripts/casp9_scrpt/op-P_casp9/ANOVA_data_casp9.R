...
cnt_combined <- bind_rows(
  lapply(names(gl_casp9.p$l_1B), function(name) {
    if (name != "Normality"){
    gl_casp9.p$l_1B[[name]]}
  }),
...
trt1_combined <- bind_rows(
  lapply(names(gl_casp9.p$l_7B), function(name) {
    if (name != "Normality"){
    gl_casp9.p$l_7B[[name]]}
  }),
  .id = "source"  # Add "source" column to track origin
)
# EXCLUDE THE AREAS
...
saveRDS(set_combined, file = "analysis/raw_rds/casp9_1B_7B_11B.op.p.rds")
