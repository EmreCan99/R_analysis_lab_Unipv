...
if (!"Normality" %in% names(df)) {
  gl_casp9.g$l_1A$Normality <- data.frame(Shapiro = NA_character_, stringsAsFactors = FALSE)
}
gl_casp9.g$l_1A$Normality$Shapiro <- ifelse(result_sh > alpha, "passed", "NOT passed")
...
gl_casp9.g$l_1A$Normality <- gl_casp9.g$l_1A$Normality %>%
...
gl_casp9.g$l_1A$Normality <- rbind(gl_casp9.g$l_1A$Normality, tempdf)
