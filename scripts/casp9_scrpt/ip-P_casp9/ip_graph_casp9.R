...
padding <- 0.012 # Add some padding above the maximum value STANDART 0
pmin <- 3.4
...
plt <- ggplot(plot_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "Casp9 1B, 7B, 11B", y = "Purkinje Cell Density", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/ip_plt/casp9_ip_1B_7B_11B.png", plot = plt, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt, file = "analysis/casp9_ip_1B_7B_11B.rds")
