...
cnt_combined <- bind_rows(
  lapply(names(gl_casp8.p$l_1B), function(name) {
    if (name != "Normality"){
    gl_casp8.p$l_1B[[name]]}
  }),
...
trt1_combined <- bind_rows(
  lapply(names(gl_casp8.p$l_7B), function(name) {
    if (name != "Normality"){
    gl_casp8.p$l_7B[[name]]}
  }),
  .id = "source"  # Add "source" column to track origin
)
# EXCLUDE THE AREAS
...
trt2_combined <- bind_rows(
  lapply(names(gl_casp8.p$l_11B), function(name) {
    if (name != "Normality"){
    gl_casp8.p$l_11B[[name]]}
  }),
  .id = "source"  # Add "source" column to track origin
)
# EXCLUDE THE AREAS
...
saveRDS(set_combined, file = "analysis/raw_rds/casp8_1B_7B_11B.op.p.rds")
