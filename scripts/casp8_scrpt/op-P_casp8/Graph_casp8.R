...
ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  theme_minimal() +
  labs(title = "One-Way ANOVA Results - casp8 1A, 7A, 11A", y = "op-Mean", x = "Condition")
...
padding <- 10 # Add some padding above the maximum value
...
plt <- ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "casp8 1B, 7B, 11B", y = "Mean Gray-Value", x = "Condition") +
  theme_classic() +
  scale_x_discrete(labels = c("CTR", "VP Pedicel", "VP Seeds")) +
  theme(
    ...
    axis.text.x = element_text(size = 15)) # Bar labels
...
ggsave(filename = "analysis/casp8_1B_7B_11B.png", plot = plt, 
       width = 12, height = 10, dpi = 300, units = "cm")

saveRDS(plt, file = "analysis/casp8_1B_7B_11B.rds")


ggsave(filename = "analysis/casp8_1B_7B_11B.sig.png", plot = plt.sig, 
       width = 12, height = 10, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/casp8_1B_7B_11B.sig.rds")
