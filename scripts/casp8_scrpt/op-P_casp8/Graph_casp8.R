...
plt <- ggplot(set_combined, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "casp8 1A, 7A, 11A", y = "Purkinje (OD)", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/op_plt/casp8_1A_7A_11A.sig.png", plot = plt.sig, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/casp8_1A_7A_11A.sig.rds")
