...
padding <- 0.090 # Add some padding above the maximum value STANDART 0
pmin <- 3.4
...
plt <- ggplot(plot_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "Bax 1A, 7A, 11A", y = "Granular Cell (CD)", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/g_ip_plt/bax_ip_1A_7A_11A.png", plot = plt.sig, 
