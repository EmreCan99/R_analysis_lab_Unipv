...
# Sink
path <- "/Users/emrecanciftci/betik/R_projects/lab_data_unipv/analysis/bcl2.ip-g.txt"
...
l_1A_list <- gl_bcl2.g$l_1A

areas <- l_1A_list[[1]][1:2, ]
df <- l_1A_list[[1]]

# Split the remaining rows into two groups: first nm.fir and last remaining
group1 <- df[3:(3 + nm.fir - 1), ]  # Rows 3 to nm.fir + 2
group2 <- df[(3 + nm.fir):nrow(df), ]  # Rows nm.fir + 2 to nrow(df)
...
saveRDS(plot_data, file = "analysis/raw_rds/bcl2_ip_1A_7A_11A_plot_data.g.rds")

saveRDS(data_sigx3, file = "analysis/raw_rds/bcl2_ip_1A_7A_11A_data_sig.g.rds")
