...
padding <- 0.009 # Add some padding above the maximum value STANDART 0
pmin <- 120
...
plt <- ggplot(plot_data, aes(x = Group, y = Mean, fill = Group)) +
  ...
  labs(title = "bcl2 1B, 7B, 11B", y = "Purkinje Cell Density", x = "Condition") +
  theme_classic() +
...
ggsave(filename = "analysis/ip_plt/bcl2_ip_1B_7B_11B.png", plot = plt.sig, 
       width = 12, height = 8, dpi = 300, units = "cm")

saveRDS(plt.sig, file = "analysis/bcl2_ip_1B_7B_11B.rds")
