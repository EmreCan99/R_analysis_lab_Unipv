...
data_combined_1A <- bind_rows(
  lapply(names(gl_bcl2.g$l_1A), function(name) {
    if (name != "Normality"){
...
if (!"Normality" %in% names(df)) {
  gl_bcl2.g$l_1A$Normality <- data.frame(Shapiro = NA_character_, stringsAsFactors = FALSE)
}
gl_bcl2.g$l_1A$Normality$Shapiro <- ifelse(result_sh > alpha, "passed", "NOT passed")
...
gl_bcl2.g$l_1A$Normality <- gl_bcl2.g$l_1A$Normality %>%
...
gl_bcl2.g$l_1A$Normality <- rbind(gl_bcl2.g$l_1A$Normality, tempdf)
